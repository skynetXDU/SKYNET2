...
#### 细节
1. SDictionary被画成一个2列的表格；
2. 可以用SDictionaryLabel指定表头显示的名称；
3. 添加按钮在最底下，删除按钮在每一行的右边，有一个叉号按钮；
4. 很多时候值字段往往比键字段复杂，默认情况下键列占总宽度的40%，值列占总宽度的60%，中间的分隔线可以左右拖动；
5. 如果字段类型太复杂，可以点击“在独立窗口打开”，会显示一个大窗口，方便编辑；
6. 大窗口里也会显示底部的两个按钮，但是目前有一个bug：如果在大窗口里编辑已有的内容，那么正常，但是如果在大窗口里新增元素并编辑，那么会卡住，关闭再打开后恢复正常；
6. 如果键和值类型有任何一个不可序列化，那么整个字典不可序列化，整个字典字段完全不显示，此时这个字段不是null，而是一个空字典；
